...
def get_position_choice(board, player_mark):
    ...
    while row_choice not in range(0, NUM_ROWS):
        row_choice = int(input("Choose your row:"))

    col_choice = int(input("Choose your column:"))

    while col_choice not in range(0, NUM_COLS):
        col_choice = int(input("Choose your column:"))

    while board[(row_choice , col_choice)] != " ":
        print()
        row_choice = int(input("Choose your row:"))
    ...
    print()
    position = (row_choice, col_choice)
...
def display_outcome(round):
    ...
    elif round <= MAX_ROUNDS and (round % 2 == 1):
        print("O wins!")
        print()
...
def is_game_complete(board):
    ...
    if check_positions(board[(0, 0)], board[(0, 1)], board[(0, 2)]):
        ...
        return True
    elif check_positions(board[(2, 0)], board[(1, 1)], board[(0, 2)]):
        return True
    else:
        return False
...
def play_tic_tac_toe(board):
    ...
    while True:
        ...
        while round < MAX_ROUNDS:
            display_board(board)
            curr_player = get_current_player(round)
            position = get_position_choice(board, curr_player)
            update_board(board, curr_player, position)
...
def is_program_finished():
    ...
    """
    plyr_response = input("Play again (Y/N)?")

    while plyr_response != "Y" and plyr_response != "y" and plyr_response != "N" and plyr_response != "n":
        plyr_response = input("Play again (Y/N)?")

    if plyr_response == "Y" or plyr_response == "y":
        print()
        return False
    if plyr_response == "N" or plyr_response == "n":
        print()
        return True
